...
// Define a structure to represent Temperature data
struct Temperature {
    float adcPinValue;
    float temp;
    char msg[20];
...
// Set RGB color values for LED based on temperature range
...
// Calibrate raw ADC value to temperature in Celsius
float tempCalibration(float raw) {
...
void readTemperature(struct Temperature *data) {
    ...
    uint32_t immediateUpdateDelay = 1; // 1 second in milliseconds
    ...
    while (1) {
        ...
        if (HAL_ADC_PollForConversion(&hadc1, 1) != HAL_OK) {
            ...
            /*##-5- Get the converted value of the regular channel  ########################*/

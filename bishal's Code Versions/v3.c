...
// Define a structure to represent an Temperature data
struct Temperature {
	float adcPinValue;
	float temp;
	char msg[20];
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {	//Calling Intruption (When there is time elapsed this fucntion is called).

	//Reading input from Push Button.
	if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6)== GPIO_PIN_RESET) {	////you have to state Pull-up instead of no-pull down below while configuring PINOUT
		//when push button is not pressed the leds will turn on.
		HAL_GPIO_WritePin(GPIOB, LD3_Pin, GPIO_PIN_SET);
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, GPIO_PIN_SET);

	} else {
		//when push button is pressed leds will remain turn off.
...
void readTemperature(struct Temperature *data) {
	 ...
	 // Check the push button status every 100 milliseconds (adjust the delay as needed)
	...
	while (1) {
		...
		if (HAL_ADC_PollForConversion(&hadc1, 1) != HAL_OK) {
			...
			HAL_Delay(1);  //3 minutes of delay before next reading
		}
...
/* In C, the . (dot) operator is used to access members of a struct or union directly,
 * while the -> (arrow) operator is used to access members of a struct or union through
 * a pointer. If you have a struct instance, you should use . to access its members. If
 * you have a pointer to a struct, you should use -> to access its members.*/

//since this is a pointer to struct
void RGB_LED(struct Temperature *data) {
	if (data->temp < -15) {
		rgbColor(WHITE);
...
int main(void)
{
  ...
  MX_TIM6_Init();
  /* USER CODE BEGIN 2 */
	HAL_TIM_Base_Start_IT(&htim6);		//initializing timer
	ssd1331_init();
